...
`PLUTO_IMAGE`              | Path to the docker image of Pluto, e.g. `eu.gcr.io/fairspace-207108/metatree-pluto:0.2.6` or `pluto-local:latest` if deploying the local build
`SATURN_IMAGE`             | Path to the docker image of Mercury, e.g. `eu.gcr.io/fairspace-207108/mercury:0.2.6` or `mercury-local:latest` if deploying the local build
`METATREE_SSL_PROXY_IMAGE` | Path to the docker image of SSL proxy, e.g. `eu.gcr.io/fairspace-207108/fairspace-ssl-proxy:0.2.6` or `fairspace-ssl-proxy-local:latest` if deploying the local build
